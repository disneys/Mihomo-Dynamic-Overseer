...
- **📡 自动化 GEO 运维：脱离客户端内置数据库限制，自动从高频更新源（Loyalsoldier）拉取最新的 geosite.dat 与 geoip.dat，并支持 24 小时静默自动更新。
- **⏱️ 全局自定义测速：统一管理所有策略组的测速逻辑，支持自定义测速地址及频率（默认 60s），确保节点切换永远基于最新延迟数据。
